...
public class GifDrawable extends Drawable implements Runnable, Animatable {
    ...
    private static final int MIN_FRAME_DELAY = 0;
    ...
    public void setmCallback(WeakReference<Callback> mCallback) {
    ...
    protected void onBoundsChange(Rect bounds) {
        ...
        mTop = bounds.top;
        mRight = bounds.right;
        mBottom = bounds.bottom;
        if (mScale) {
            mScaleFactor = Math.min((float) mWidth / mIntrinsicWidth,
    ...
    public void draw(Canvas canvas) {
        ...
        if (mScale) {
            ...
            //canvas.drawBitmap(mBitmap, mLeft/mScaleFactor, mTop/mScaleFactor, sScalePaint);
            canvas.drawBitmap(mBitmap, new Rect(0,0,mBitmap.getWidth(),mBitmap.getHeight()),new Rect(mLeft,mTop,mRight,mBottom), sScalePaint);
            canvas.restore();
        } else {
        ...
        if (!mRunning && !mDone) {
            start();
            //Log.i("gifDrawable","start thread");
        }
    }
    /**
     * {@inheritDoc}
